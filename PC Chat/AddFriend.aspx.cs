...
public partial class AddFriend : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //SqlDataAdapter da = new SqlDataAdapter("SELECT ID, Name, Mobile, EMail FROM PCMember WHERE (ID <> '" + id + "') AND (Name NOT IN (SELECT Name2 FROM PCContact WHERE (Name1 = '" + Session["mname"].ToString() + "'))) AND (Name NOT IN (SELECT Name1 FROM PCContact WHERE Name2 = '" + Session["mname"].ToString() + "'))", con);
    ...
    protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Add")
        {
            ...
            cmd = new SqlCommand("Insert into PCContact Values ('"+nid+"','"+nid+"','"+name+"','"+im+ "','" + i1 + "','" + img + "','','No')", con);
            con.Open();
            cmd.ExecuteNonQuery();
            con.Close();
            Session["AddN"] = "Yes";
    ...
    {
        string id = Session["mid"].ToString();
        SqlDataAdapter da = new SqlDataAdapter("SELECT ID, Name, Mobile, EMail FROM PCMember WHERE (ID <> '" + id + "') AND (Name Like '%" + TextBox1.Text + "%') AND (Name NOT IN (SELECT Name2 FROM PCContact WHERE (Name1 = '" + Session["mname"].ToString() + "'))) AND (Name NOT IN (SELECT Name1 FROM PCContact WHERE Name2 = '" + Session["mname"].ToString() + "'))", con);
        DataSet ds = new DataSet();
        da.Fill(ds);
        GridView1.DataSource = ds;
        GridView1.DataBind();
